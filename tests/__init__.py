"""Unit test package for fda_workflow."""
...
#     if real:
#         return draw(builds(Path, builds(lambda: tempfile.mkdtemp())))
#     r = [just(under)]
...
@composite
def paths(draw, real=False, *a, **k):
    if real:
        return draw(builds(Path, builds(lambda: tempfile.mkdtemp())))
    return draw(builds(Path, names()))
...
names = lambda: text(alphabet=characters(blacklist_characters='/\^* ',
                                         min_codepoint=33,
                                         max_codepoint=126), min_size=5, max_size=10)
